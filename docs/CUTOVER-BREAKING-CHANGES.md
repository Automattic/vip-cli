...
> conducted against this repo's *vendored* `src/`, which was (a) frozen at 4.0.4 while upstream
...
| 1.10 | Error output | adds a second space after `Error:`, always writes a runtime `Debug:` line to **stdout**, and may dump the stack when debug namespaces are enabled | one-space `Error:` message on stderr, no runtime banner or stack on stdout | anyone parsing stdout on failure — note this *fixes* corrupt `--format json` output |
...
| 1.23 | `import sql --search-replace` **with `--in-place`** | applies the pairs **twice** — the rewritten file is uploaded *and* the pairs are sent to the server (`vip-import-sql.js:760` is not gated on `isUrl`) | applied once | anyone whose replacement is non-idempotent, e.g. `a,aa` turned "a" into "aaaa". A domain swap hides the bug, which is why it survived. URL imports and local imports *without* `--in-place` are unchanged. |
...
**Decided exception — do NOT keep:** `config software update` rejecting *deprecated* versions.
...
*any* mutation whose response carries `elevated-permission-required`. The moment the API enables
...
*Fixed since this list was written:* the `--search-replace` double-apply (now 1.23), the
