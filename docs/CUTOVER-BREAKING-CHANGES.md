...
| ~~2.15~~ | ~~`VIP_TOKEN_OVERRIDE` honored outside `NODE_ENV=test`~~                                                                            | —                                                                    | **DELETED — VOID.** The variable never existed in Node; it had been hand-injected into the vendored `src/`. This compared Go against fabricated behavior. `33790a72` gated Go's override on `GO_ENV`/`NODE_ENV=test` and was described as parity; it is not, and the gate was retained temporarily as a Go-only hardening decision. The override has now been removed: general tests use the supported `VIP_CLI_TOKEN` source in both runtimes, while stored-session tests seed isolated credentials. |
