...
- `VIP_CLI_TOKEN` authenticates both runtimes. General command tests use a valid synthetic PAT through that variable; stored-session tests must clear it and use an injected store or the parity harness's isolated credential fixtures. The former Go-only `VIP_TOKEN_OVERRIDE` is no longer supported. Do not use dummy deploy tokens as a general-purpose auth bypass.
...
- **Go prereleases:** 5.x alpha/beta/rc releases use `.github/workflows/vip-next-prerelease.yml`. It creates an exact-SHA public tag, leaves the GitHub release as a draft while the matching Buildkite tag build signs both binaries, verifies the exact artifact set, and publishes only after upload. Dispatching it is a live release action and requires explicit user authorization.
