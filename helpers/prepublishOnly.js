    ...
    try {
        ...
        if ( config.gitBranch ) {
            ...
            if ( currentBranch.trim() !== config.gitBranch ) {
                return bail( `Git branch is not ${ config.gitBranch }. Please switch to ${ config.gitBranch } before publishing.` );
            }
        }
        ...
        if ( config.nodeEnforceVersion ) {
            ...
            if ( !isSatisfied ) {
                return bail( `Node version ${versions.node.version} is not supported. Please use Node version ${ config.nodeEnforceVersion } or higher.` );
            }
        }
