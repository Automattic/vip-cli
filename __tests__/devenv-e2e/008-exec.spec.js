...
describe( 'vip dev-env exec', () => {
	...
	describe( 'if the environment does not exist', () => {
		it( 'should fail', async () => {
			...
			const result = await cliTest.spawn( [ process.argv[ 0 ], vipDevEnvExec, '--slug', slug ], {
				env,
			} );
	...
	describe( 'if the environment exists', () => {
		...
		it( 'should run known commands', async () => {
			const result = await cliTest.spawn(
				[ process.argv[ 0 ], vipDevEnvExec, '--slug', slug, '--', 'wp', 'option', 'get', 'home' ],
				{ env },
				true
			);
		...
		it( 'should fail on unknown commands', async () => {
			const result = await cliTest.spawn(
				[ process.argv[ 0 ], vipDevEnvExec, '--slug', slug, '--', 'ls' ],
				{ env }
			);
			expect( result.rc ).toBeGreaterThan( 0 );
