...
const INSERT_STATEMENT_MODIFIERS = new Set( [
	'IGNORE',
	'LOW_PRIORITY',
	'DELAYED',
	'HIGH_PRIORITY',
...
interface InsertStatementInfo {
	tableName: string;
	tableEndIndex: number;
}

const DEFAULT_OPTIONS_INSERT_COLUMNS = [ 'option_id', 'option_name', 'option_value', 'autoload' ];

const skipSqlWhitespace = ( line: string, startIndex: number ): number => {
...
const readSqlIdentifier = ( line: string, startIndex: number ): SqlIdentifier | undefined => {
...
const getInsertStatementInfo = ( line: string ): InsertStatementInfo | undefined => {
...
const isWordPressOptionsTable = ( tableName: string | undefined ): boolean => {
...
const checkRequiresOptionsInsertContext = ( checkKey: string ): boolean => {
...
const findValuesKeywordIndex = ( line: string ): number => {
	const valuesMatches = /\bVALUES\b/i.exec( line );
	return valuesMatches?.index ?? -1;
};

const parseInsertColumnList = ( line: string, startIndex: number ): string[] | undefined => {
...
const parseInsertColumnListSegment = ( columnList: string ): string[] | undefined => {
...
const isEscapedByBackslash = ( line: string, index: number ): boolean => {
...
interface SqlTupleRowsParseResult {
	rows: string[][];
	remainder?: string;
}
...
const stripSqlCommentsOutsideQuotedStrings = ( line: string ): string => {
	let uncommentedLine = '';
...
const parseSqlTupleRows = ( line: string, startIndex: number ): SqlTupleRowsParseResult => {
	const rows: string[][] = [];
...
const unquoteSqlValue = ( value: string | undefined ): string => {
...
const getOptionUrlMatchResults = ( row: string[], columns?: string[] ): string[] | undefined => {
