...
import { validate as validateSQL } from '../validations/sql';
import {
...
import { getAllEnvironmentNames, getEnvironmentPath, getVersionList, readEnvironmentData, resolveImportPath, exec } from './dev-environment-core';
import type {
	AppInfo,
	ComponentConfig,
	InstanceOptions,
...
import { validateDockerInstalled, validateDockerAccess, bootstrapLando, isEnvUp } from './dev-environment-lando';
...
export class DevEnvSQLImportCommand {
	fileName;
	options;
	slug;
	trackingInfo;
	...
	async run() {
		...
		if ( ! this.options.skipValidate ) {
			if ( ! isEnvUp( lando, getEnvironmentPath( this.slug ) ) ) {
